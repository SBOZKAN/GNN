...
def main():
    ...
    for fold in range(kfold):
        ...
        seq_train=train.loc[:,train.columns[3:266]].values
        seq_train=[''.join(i) for i in seq_train]
        seq_valid=valid.loc[:,valid.columns[3:266]].values
        seq_valid=[''.join(i) for i in seq_valid]

        feature_train=train.loc[:,"26_1":"290_3"]
        feature_valid=valid.loc[:,"26_1":"290_3"]
        X_train=enc.encode(encoding=encoder, char_seqs=list(seq_train), feature_data=feature_train)
        X_valid=enc.encode(encoding=encoder, char_seqs=list(seq_valid), feature_data=feature_valid)
        ...
        seq_test=test.loc[:,test.columns[3:266]].values
        seq_test=[''.join(i) for i in seq_test]
        feature_test=test.loc[:,"26_1":"290_3"]
        #if encoder in ['one_hot', 'aa_index', 'one_hot,aa_index']:
        X_test=enc.encode(encoding=encoder, char_seqs=list(seq_test), feature_data=feature_test)
