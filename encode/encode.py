...
def enc_aa_index2(seqs):
    "using 16 features directly without PCA"
    aa_index2=pd.read_csv('data/aaindex/AminoAcidChemistry.csv', header=1)
    aa_index2=aa_index2.loc[:19, aa_index2.columns[:17]]
    
    index_map=defaultdict()
    for i,row in aa_index2.iterrows():
        aa=row.values[0]
        emb=np.array(row.values[1:], dtype=np.float32)
        index_map[aa]=emb
...
# def enc_bert(seqs,dim=128):
    
#     bm = ProtTransBertBFDEmbedder()
#     bert_enc=[bm.embed(s)[:,:dim] for s in seqs]
#     return np.array(bert_enc)
...
def encode(encoding, char_seqs=None, variants=None, ds_name=None, wt_aa=None, wt_offset=None):
    ...
    for enc in encodings:
        if enc == "one_hot":
            ...
            encoded_data.append(enc_aa_index2(char_seqs))
        elif enc == "bert":
            encoded_data.append(enc_bert(char_seqs))
        elif enc == "prot_vec":
            encoded_data.append(enc_prot_vec(char_seqs))
        else:
            raise ValueError("err: encountered unknown encoding: {}".format(enc))
